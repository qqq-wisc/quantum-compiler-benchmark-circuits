...
pm = PassManager([Decompose(), UnrollCustomDefinitions(equivalence_library=sel,basis_gates=['cx', 'rz', 'h']), BasisTranslator(sel, ['cx', 'rz', 'h']), SolovayKitaev(recursion_degree=2)])
compiled = pm.run(qc)
compiled.qasm(filename='grover.qasm')